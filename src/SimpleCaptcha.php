...
class SimpleCaptcha {
    ...
    public function CreateImage($token) {
        ob_clean();
        $ini = microtime(true);
	...
	//写入 redis
	public function writeRedis($token , $value ){
		$redis = RedisClass::getInstance(0);
		$redis->SET($token, $value, 300 );
	}
	...
	public function checkCode($token , $code ){
		...
		if(strtolower(trim($value)) == strtolower(trim($code))){
			return true ;
		}
		return false;
	}
